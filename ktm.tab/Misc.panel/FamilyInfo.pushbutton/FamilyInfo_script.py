...
def GetVal(Type, Param, doc):
    ...
    info_items.append(DB.LabelUtils.GetLabelFor(Param.parameter_group))
    ...
    try:
        info_items.append(DB.LabelUtils.GetLabelFor(Param.parameter_type))
    except:
...
if paths:
    max_value = len(paths)
    with forms.ProgressBar(title='Анализ... ({value} из {max_value})', cancellable=True) as pb:
